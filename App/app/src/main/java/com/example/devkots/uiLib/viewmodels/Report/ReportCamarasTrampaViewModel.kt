...
class ReportCamarasTrampaViewModel(
    ...
    private fun fetchReportFromDatabase(reportId: Int) {
        viewModelScope.launch {
            loading = true
            try {
                CamarasTrampaReportDao?.getCamarasTrampaReportById(reportId.toLong())?.let {
                    report = it.toCamarasTrampaReport()
                }
    ...
    fun updateReport(reportId: Int, updatedReport: CamarasTrampaReport) {
        viewModelScope.launch {
            ...
            try {
                ...
                CamarasTrampaReportDao?.updateCamarasTrampaReport(entity)
